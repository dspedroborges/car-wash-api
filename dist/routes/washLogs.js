...
router.get("/car/:id", authenticate, async (req, res) => {
    if (!req.isAdmin)
        return res.status(403).json({ message: "Acesso negado" });
...
router.post("/", authenticate, upload.fields([
    ...
    try {
        ...
        if (!activeDebt)
            return res.status(404).json({ message: "Nenhum débito ativo encontrado" });
        if (!activeDebt.washType || !activeDebt.package)
...
//# sourceMappingURL=washLogs.js.map