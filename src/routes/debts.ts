...
router.get("/:id", authenticate, async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (!debt) return res.status(404).json({ message: "Débito não encontrado" });

  res.status(200).json(debt);
});

router.get("/client/:id", authenticate, async (req: AuthenticatedRequest, res: Response) => {
  ...
  else if (filter === "canceled") where.status = "canceled";
  else if (filter === "expired") where.expiresAt = { lt: new Date() };
...
router.post("/", authenticate, async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (Number(discount) > 50) return res.status(400).json({ message: "Desconto excessivo" });
