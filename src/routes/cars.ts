...
router.get("/client/:id", authenticate, async (req: AuthenticatedRequest, res) => {
...
router.post("/", authenticate, upload.array("images"), async (req: AuthenticatedRequest, res) => {
    ...
    if (existing) return res.status(400).json({ message: "Já existe um carro com essa placa" });
...
router.put("/:id", authenticate, upload.array("images"), async (req: AuthenticatedRequest, res) => {
    const carId = Number(req.params.id);
